...
export default class FLK extends Component {
    ...
    renderForm = () => {
        const { masterData, dataFLK, handleStateFlk, source, handleStateForm2Flk, fetchSubmitFLK, RecruitmentPhase, isFLKSubmit, resetActivityPriority, resetJobPriority, setReadMore } = this.props
        let detail = window.location.pathname
        if (this.state.posisi === 1) {
            ...
            return (
                <Form3
                    ...
                    RecruitmentPhase={RecruitmentPhase}
                    isFLKSubmit={isFLKSubmit}
                    resetActivityPriority={resetActivityPriority}
                    resetJobPriority={resetJobPriority}
                />
            )
        } else {
